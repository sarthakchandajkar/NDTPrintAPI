...
public sealed class NdtBatchStateService : INdtBatchStateService
{
    ...
    public async Task<(int BatchNumber, int TotalSoFar, int Threshold)> GetBatchForRecordAsync(
        string poNumber,
        int millNo,
        int ndtPipes,
        CancellationToken cancellationToken)
    {
        ...
        var threshold = await ResolveThresholdAsync(poNumber, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<int> ResolveThresholdAsync(string poNumber, CancellationToken cancellationToken)
    {
