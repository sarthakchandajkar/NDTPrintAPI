...
public static class ZplNdtLabelBuilder
{
        ...
        NdtTagLabelSize size)
    {
        ...
        var fieldWidth = size.WidthDots - (margin * 2);
        const int topBarcodeHeight = 145;
        const int bottomBarcodeHeight = 100;
        const int barcodeHumanReadable = 34;
        const int bottomGap = 16;

        var bottomY2 = size.LengthDots - margin - bottomBarcodeHeight - barcodeHumanReadable;
        ...
        var hasStation = !string.IsNullOrWhiteSpace(stationText);
        var textLineCount = hasStation ? 5 : 4;
        var lineStep = Math.Max(46, (textEndY - textStartY) / textLineCount);
        ...
        var y = margin;
        zpl.AppendFormat("^FO{0},{1}^BY3^BCN,{2},Y,N,N^FD{3}^FS", left, y, topBarcodeHeight, escapedBatch);
        y = textStartY;
        ...
        zpl.AppendFormat("^FO{0},{1}^BY3^BCN,{2},Y,N,N^FD{3}^FS", left, bottomY1, bottomBarcodeHeight, escapedBatch);
        zpl.AppendFormat("^FO{0},{1}^BY3^BCN,{2},Y,N,N^FD{3}^FS", left, bottomY2, bottomBarcodeHeight, escapedBatch);
